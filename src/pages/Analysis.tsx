...
export default function Analysis({ transactions }: Props) {
  ...
  const now = transactions.filter((t) => t.type === 'expense' && isInPeriod(t.date, unit, period))
  const nowIncome = transactions.filter((t) => t.type === 'income' && isInPeriod(t.date, unit, period))
  const prevPeriod = previousPeriod(unit, period)
  const prevExpenses = transactions.filter((t) => t.type === 'expense' && isInPeriod(t.date, unit, prevPeriod))
  ...
  const topMerchants = Object.entries(merchantCounts).sort((a, b) => b[1].count - a[1].count).slice(0, 5)
  ...
  const recurringCandidates = detectRecurring(transactions.filter((t) => t.type === 'expense'))
  ...
  return (
    <>
        ...
        <div className="period-bar">
          <div className="period-units">
            {(['year', 'month', 'week', 'day'] as PeriodUnit[]).map((u) => (
              <button key={u} className={u === unit ? 'punit active' : 'punit'} onClick={() => switchUnit(u)}>
      ...
      <section className="card">
        <div className="section-head"><h2>支出</h2></div>
        <div className="balance">{yen(total)}</div>
        <p className="sub">{prevTotal ? `前${unitLabel(unit)}比 ${diff >= 0 ? '+' : ''}${yen(diff)}` : `前${unitLabel(unit)}データがありません`}</p>
      </section>

      <section className="card">
        <div className="section-head"><h2>収支バランス</h2></div>
        <div className="stats">
          <div className="stat"><label>収入</label><strong>{yen(totalIncome)}</strong></div>
          <div className="stat"><label>支出</label><strong>{yen(total)}</strong></div>
        </div>
        <p className="sub">{savingsRate !== null ? `貯蓄率 ${savingsRate.toFixed(1)}%` : 'この期間の収入データがありません'}</p>
      </section>

      <section className="card">
        <div className="section-head"><h2>カテゴリ別支出</h2></div>
        {categoryRanking.length === 0 && <div className="empty">データがありません。</div>}
        {categoryRanking.length > 0 && (
          <div className="donut-wrap">
            <svg viewBox="0 0 100 100" className="donut">
              {donutSegments.map((seg, i) => (
                <circle key={seg.name} cx="50" cy="50" r="40" fill="none" stroke={colorFor(seg.name, i)} strokeWidth="16" strokeDasharray={`${seg.dash} ${seg.gap}`} strokeDashoffset={seg.offset} transform="rotate(-90 50 50)" />
              ))}
            </svg>
            <div className="donut-center"><div className="donut-total">{yen(total)}</div></div>
          </div>
        )}
        {categoryRanking.map(([name, info], i) => (
          <div className="spend-row" key={name}>
            ...
            <div className="grow"><div className="name">{info.icon ? `${info.icon} ` : ''}{name}</div><div className="sub">{total > 0 ? ((info.total / total) * 100).toFixed(1) : '0'}%</div></div>
      ...
      <section className="card">
        ...
        {categoryComparison.map((c) => (
          <div className="spend-row" key={c.name}>
            <div className="grow"><div className="name">{c.name}</div><div className="sub">前{unitLabel(unit)} {yen(c.prev)} → 今{unitLabel(unit)} {yen(c.current)}</div></div>
            <div className={c.diff > 0 ? 'amount up' : c.diff < 0 ? 'amount down' : 'amount'}>{c.diff >= 0 ? '+' : ''}{yen(c.diff)}</div>
          </div>
        ))}
      </section>

      <section className="card">
        <div className="section-head"><h2>支出頻度</h2></div>
        <div className="stats"><div className="stat"><label>利用回数</label><strong>{count}回</strong></div><div className="stat"><label>平均支出額</label><strong>{yen(avgAmount)}</strong></div></div>
        <p className="sub">{maxTx ? `最大支出：${maxTx.merchants?.canonical_name || '未設定'}（${yen(maxTx.amount)}）` : 'データがありません'}</p>
      </section>

      <section className="card">
        <div className="section-head"><h2>小額支出</h2></div>
        <p className="sub">小額支出：{small.length}回 / {yen(small.reduce((a, t) => a + t.amount, 0))}</p>
        <p className="sub">500円以下の利用回数と合計です。</p>
      </section>

      <section className="card">
        <div className="section-head"><h2>よく利用する店舗</h2></div>
        {topMerchants.length === 0 && <div className="empty">データがありません。</div>}
        {topMerchants.map(([name, info]) => (
          <div className="spend-row" key={name}><div className="grow"><div className="name">{name}</div><div className="sub">{info.count}回</div></div><div className="amount">{yen(info.total)}</div></div>
        ))}
      </section>

      <section className="card">
        <div className="section-head"><h2>定期支出候補</h2></div>
        <div className="empty">まだ十分なデータがありません。</div>
        {recurringCandidates.map((c) => (
          <div className="spend-row" key={c.merchant}><div className="grow"><div className="name">{c.merchant}</div><div className="sub">約{c.intervalDays}日ごと・平均{yen(c.avgAmount)}／定期的に発生している可能性があります</div></div></div>
        ))}
      </section>
    </>
  )
...
interface DonutSegment { name: string; dash: number; gap: number; offset: number }

function buildDonutSegments(ranking: [string, { total: number; icon: string | null }][], total: number): DonutSegment[] {
...
interface RecurringCandidate { merchant: string; intervalDays: number; avgAmount: number }
...
function detectRecurring(expenses: TxRow[]): RecurringCandidate[] {
  ...
  Object.entries(byMerchant).forEach(([name, txs]) => {
    ...
    for (let i = 1; i < sorted.length; i++) {
      ...
      if (days >= 25 && days <= 35 && amtRatio <= 0.1) { intervals.push(days); amounts.push(sorted[i].amount, sorted[i - 1].amount) }
    }
